...
点击 Activity Bar 的齿轮图标，打开全屏设置浮层：
- 通用：版本更新检测、调试日志
- 外观：主题、强调色、标题栏样式、底栏样式、语言
- 终端：字体、字号、光标样式、配色方案
- 监控：系统刷新间隔、延迟检测间隔、流量监控、地理位置查询
...
- **设置浮层** — 设置面板改为全屏浮层覆盖，不切换侧边栏，关闭后回到之前页面
- **设置内 Tab 栏** — 子页面切换从侧栏图标改为顶部文字标签，跟随语言切换
- **后端安全加固** — exec 命令检查退出状态码、semaphore 获取超时、终端读循环空闲超时、chmod 仅允许八进制、URL 严格校验、根路径规范化防绕过
- **前端稳定性** — FileManager / StatusBar 事件监听清理修复，避免组件卸载时泄漏
- **构建优化** — 主 JS 分包动态加载（~1MB → 187KB），首次加载大幅提速
- **Ping 速率限制** — 后端 2 秒冷却，避免重复 ping 堆叠
- **配置原子保存** — 写入临时文件 + rename，防止配置损坏
- **翻译完善** — 补全中文设置描述，英文文案趋近干净
- **右键菜单** — 全局拦截改为仅 #app 容器，允许输入框右键
