...
watch(() => props.visible, (v) => { log.info('visible changed', v) }, { immediate: true })
          ...
          <div class="p-5 space-y-6">
            ...
            <section>
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  <div class="flex rounded-lg bg-[var(--color-bg-secondary)] p-0.5">
                    <button v-for="th in ['light','system','dark']" :key="th"
                      @click="settings.setTheme(th)" :class="segBtnClass(settings.theme === th)"
                  ...
                  <div class="flex gap-2">
                    <button v-for="c in accentColors" :key="c"
                      ...
                      :class="['w-8 h-8 rounded-full transition-all hover:scale-110',
                        settings.accentColor === c ? 'ring-2 ring-offset-2 ring-offset-[var(--color-bg-primary)] scale-110' : '']"
                ...
                <div>
                  ...
                  <div class="flex rounded-lg bg-[var(--color-bg-secondary)] p-0.5">
                    <button @click="settings.setTitlebarStyle('macos')" :class="segBtnClass(settings.titlebarStyle === 'macos')">
                      <span class="flex items-center justify-center gap-1">
                        <span class="flex gap-1"><span class="w-2 h-2 rounded-full bg-[#FF5F57]"/><span class="w-2 h-2 rounded-full bg-[#FFBD2E]"/><span class="w-2 h-2 rounded-full bg-[#28C840]"/></span>
                        {{ t('settings.titlebarMacos') }}
                      </span>
                    </button>
                    <button @click="settings.setTitlebarStyle('win11')" :class="segBtnClass(settings.titlebarStyle === 'win11')">
                ...
                <div>
                  ...
                  <div class="flex rounded-lg bg-[var(--color-bg-secondary)] p-0.5">
                    <button @click="settings.setStatusbarStyle('default')" :class="segBtnClass(settings.statusbarStyle === 'default')">{{ t('settings.statusbarDefault') }}</button>
                    <button @click="settings.setStatusbarStyle('accent')" :class="segBtnClass(settings.statusbarStyle === 'accent')">{{ t('settings.statusbarAccent') }}</button>
                  </div>
                </div>
                  ...
                  <div class="grid grid-cols-2 gap-2">
                    <button v-for="[key, p] in presets" :key="key" @click="selectPreset(key)"
                      :class="['flex items-center gap-2 p-2 rounded-lg transition-all text-left',
                        settings.terminalColorPreset === key ? 'ring-2 ring-[var(--color-accent)] bg-[var(--color-bg-secondary)]' : 'hover:bg-[var(--color-bg-secondary)]']">
                      <div class="w-8 h-5 rounded-sm border border-[var(--color-border)] overflow-hidden shrink-0" :style="{ background: p.background }">
            ...
            <section>
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  <input v-model="settings.fontFamily" @change="settings.save()" :class="inputClass()" placeholder="Consolas, monospace" />
                </div>

                <div :class="settingRowClass()">
                  <label class="text-[11px] font-medium text-[var(--color-text-secondary)]">{{ t('settings.fontSize') }}</label>
                  <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    <span class="text-xs tabular-nums text-[var(--color-text-primary)] w-9 text-right font-mono">{{ settings.fontSize }}px</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div class="flex rounded-lg bg-[var(--color-bg-secondary)] p-0.5">
                    <button v-for="c in ['block','bar','underline']" :key="c"
                      @click="settings.cursorStyle = c; settings.save()" :class="segBtnClass(settings.cursorStyle === c)"
                    >{{ t(`settings.${c}`) }}</button>
                  </div>
                </div>
            ...
            <section>
              ...
              <div class="space-y-2.5">
                ...
                <div :class="settingRowClass()">
                  <label class="text-[11px] font-medium text-[var(--color-text-secondary)]">{{ t('settings.sysRefresh') }}</label>
                  <div class="flex items-center gap-1.5">
                    <input type="number" min="3" max="30" :value="settings.monitorRefreshSecs"
                ...
                <div :class="settingRowClass()">
                  <label class="text-[11px] font-medium text-[var(--color-text-secondary)]">{{ t('settings.pingInterval') }}</label>
                  <div class="flex items-center gap-1.5">
                    <input type="number" min="3" max="120" :value="settings.pingIntervalSecs"
                ...
                <div :class="settingRowClass()">
                  <label class="text-[11px] font-medium text-[var(--color-text-secondary)]">{{ t('settings.showTraffic') }}</label>
            ...
            <section>
              ...
              <div class="space-y-2.5">
                <div :class="settingRowClass()">
                  <label class="text-[11px] font-medium text-[var(--color-text-secondary)]">{{ t('settings.checkUpdates') }}</label>
                  <button role="switch" :aria-checked="settings.checkUpdates"
                    @click="settings.checkUpdates = !settings.checkUpdates; settings.save()"
                    :class="['relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200',
                      settings.checkUpdates ? 'bg-[var(--color-accent)]' : 'bg-[var(--color-bg-tertiary)]']">
                    <span :class="['pointer-events-none inline-block h-4 w-4 rounded-full bg-white shadow transition-transform duration-200',
                      settings.checkUpdates ? 'translate-x-4' : 'translate-x-0']" />
                  </button>
                </div>
              </div>
            </section>
